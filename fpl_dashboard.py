...
# ----------------------------------------------------------------------
st.set_page_config(
    page_title="Premier League Dashboard - Redone",
    layout="wide",
...
if "club_colors" not in st.session_state:
    st.session_state["club_colors"] = {}
...
def prepare_data(data):
    ...
    We rename some columns to give the app a new feel.
    """
    if not data:
    ...
    # If any are empty, return them directly
    if players_df.empty or clubs_df.empty:
        return players_df, clubs_df

    # Rename columns to freshen the look
    # 'second_name' -> 'last_name'
    # 'team' -> 'club_id' (for merging)
    # 'now_cost' -> 'cost'
    ...
    players_df.rename(columns={"minutes": "minutes_played"}, inplace=True)
...
def assign_colors_to_clubs(players_df, clubs_df):
    """
    Create a color map for each club. We can either:
      - Assign random pastel colors, OR
      - Use a custom color palette if you prefer.

    This example randomly picks from a pastel range.
    """
    if clubs_df.empty or "name" not in clubs_df.columns:
        return {}

    club_names = clubs_df["name"].dropna().unique().tolist()

    # Let's define a helper function to get random pastel colors
    def random_pastel_color():
        # Create pastel colors by limiting saturation
        r = 200 + random.randint(0, 55)
        g = 200 + random.randint(0, 55)
        b = 200 + random.randint(0, 55)
        return f"rgb({r},{g},{b})"

    color_map = {}
    for club_name in club_names:
        color_map[club_name] = random_pastel_color()
...
def refresh_data():
    ...
    st.session_state["club_colors"] = assign_colors_to_clubs(p_df, c_df)

# ----------------------------------------------------------------------
# 5. Main App Layout: Using Tabs
# ----------------------------------------------------------------------
def tab_overview(players_df):
    ...
    # Show top 30 players by total_points
    ...
    fig = px.bar(
        ...
        y="total_points",
        color="club",
        color_discrete_map=st.session_state["club_colors"] or {},
...
def tab_fixtures(clubs_df):
    ...
    try:
        ...
        if filter_club != "All":
            fix_df = fix_df[(fix_df["Home"] == filter_club) | (fix_df["Away"] == filter_club)]
...
def tab_best_players(players_df):
    ...
    relevant_metrics = position_map.get(chosen_position, [])
...
# 6. Run App
# ----------------------------------------------------------------------
# Refresh or load data if empty
if st.session_state["players"].empty or st.session_state["clubs"].empty:
    refresh_data()

st.title("Premier League Dashboard (All New!)")
...
# Allow user to refresh data via a button
st.write("---")
